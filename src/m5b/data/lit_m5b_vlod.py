...
class LitM5BVLODWDS(LightningDataModule):
    def __init__(
        self,
        ...
        choices_type: Literal["alpha", "numeric"] = "alpha",
        prompt_template: str = (
            "Based on the {NUM_IMGS} images ordered from top-left to bottom-right, "
            "which image does not match the hypothesis ”{HYPOTHESIS}”? Choose one from {CHOICES}:"
        ),
        ...
        if use_stacked_images not in ["vertically", "horizontally", "two_cols"]:
            raise ValueError(
                f"Invalid value for use_stacked_images: {use_stacked_images}"
            )
        self.use_stacked_images = use_stacked_images

        self.choices_type = choices_type
        self.prompt_template = self.build_preliminary_prompt_template(prompt_template)
    ...
    def build_preliminary_prompt_template(
        self,
        prompt_template: str,
    ) -> str:
        if "{NUM_IMGS}" not in prompt_template:
            raise ValueError("Prompt template must contain '{NUM_IMGS}'")
        if "{HYPOTHESIS}" not in prompt_template:
            raise ValueError("Prompt template must contain '{HYPOTHESIS}'")
        if "{CHOICES}" not in prompt_template:
            raise ValueError("Prompt template must contain '{CHOICES}'")

        if self.choices_type == "alpha":
            choices = "[A, B, C, D, E]"
        elif self.choices_type == "numeric":
            choices = "[1, 2, 3, 4, 5]"
        else:
            raise ValueError(f"Invalid value for choices_type: {self.choices_type}")
        return prompt_template.replace("{NUM_IMGS}", "5").replace("{CHOICES}", choices)
    ...
    def prepare_data(self):
        ...
        for idx, lang in enumerate(self.languages):
            ...
            log.info(f"Built M5D VLOD WDS dataloader for language '{lang}'!")
    ...
    def _build_dataloader(self, lang: str) -> wds.WebLoader:
        ...
        def _collate_fn(batch):
            ...
            if self.use_stacked_images == "vertically":
                ...
                images = batch[1]
            elif self.use_stacked_images == "two_cols":
                images = batch[2]
            else:
                raise ValueError(
            ...
            hypotheses = list(map(_decode, batch[3]))
            label_strs = list(map(_decode, batch[4]))
            topics = list(map(_decode, batch[5]))
            languages = list(map(_decode, batch[6]))
            sample_ids = list(map(_decode, batch[7]))
