...
class LitxFlickrCOMWDS(LightningDataModule):
    def __init__(
        self,
        ...
        prompt_template: str = "Brief caption in {LANGUAGE}:",
    ...
    def prepare_data(self):
        ...
        for idx, lang in enumerate(self.languages):
            ...
            log.info(f"Built xFlickrCO WDS dataloader for language '{lang}'!")
    ...
    def _build_dataloader(self, lang: str) -> wds.WebLoader:
        ...
        def _collate_fn(batch):
            ...
            sample_ids = list(map(_decode, batch[3]))
            keys = list(map(_decode, batch[4]))

            languages = [self.languages[lang] for lang in langs]
            prompts = [
                self.prompt_template.format(LANGUAGE=language) for language in languages
            ]
